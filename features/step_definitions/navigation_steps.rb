...
#When /^I (press|follow|check|uncheck|choose) "([^\"]*)" the comment whose (.*) is "([^\"]*)"$/ do |action, whatyouclick, var_name, value|
#  unless var_name == "id" then
#    id = Comment.find_by_body("\"Keep up the good work\"").dom_id
#    #id = (eval("Comment.find_by_body(\"#{value}\").dom_id"))
#  else
#    id = value
#  end
#  within("tr[id=#{id}]") do
